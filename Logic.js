const createNewAccount = async (account_name,code) => {const data = {account_name,Balance: 0,code}
  return await fetch("https://json-server-4b1p.onrender.com/users", {
    method: "POST",
    headers: {
...
const deposit = async (code,account) => {
  let data = await fetch("https://json-server-4b1p.onrender.com/users");
  ...
  async function depositing(x){
    ...
    const data = {
      ...
      return await fetch(`https://json-server-4b1p.onrender.com/users/${id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
...
const withdraw=async (code,amount)=>{
   ...
   else if(Balance<amount){console.log(`❌ Please Recharge Your wallet with ${Math.abs(Balance-amount)}₹`)}
}
